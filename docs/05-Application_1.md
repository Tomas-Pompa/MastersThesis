...
Nyní se věnujme aplikaci na reálných datech. Budeme pracovat s daty `growth()`, které jsou dostupné v knihovně `fda`.
...
<img src="05-Application_1_files/figure-html/unnamed-chunk-3-1.png" width="672" />
...
<div class="figure">
<img src="05-Application_1_files/figure-html/unnamed-chunk-6-1.png" alt="Průběh $GCV(\lambda)$ pro zvolený vektor $\boldsymbol\lambda$. Na ose $x$ jsou hodnoty vyneseny v logaritmické škále. Červeně je znázorněna optimální hodnota vyhlazovacího parametru $\lambda_{optimal}$." width="672" />
<p class="caption">(\#fig:unnamed-chunk-6)Průběh $GCV(\lambda)$ pro zvolený vektor $\boldsymbol\lambda$. Na ose $x$ jsou hodnoty vyneseny v logaritmické škále. Červeně je znázorněna optimální hodnota vyhlazovacího parametru $\lambda_{optimal}$.</p>
</div>
...
DFsmooth <- data.frame(
  ...
  Gender = factor(rep(c('girls', 'boys'), each = n * length(t)),
                  levels = c('girls', 'boys'))
)
...
DFsmooth |> ggplot(aes(x = t, y = Smooth, color = Gender, group = time)) + 
  ...
  labs(x = 'Time',
       y = 'Function',
       colour = 'Gender') +
  scale_colour_discrete(labels = c('girls', 'boys')) + 
...
##      1      2      3      4      5      6      7      8 
## 0.8769 0.8923 0.9385 0.9538 0.9538 0.9385 0.9538 0.9385 
## 
## -Optimal number of neighbors: knn.opt= 4 
## with highest probability of correct classification max.prob=  0.9538462 
## 
## -Probability of correct classification:  0.9538
```
...
<img src="05-Application_1_files/figure-html/unnamed-chunk-13-1.png" width="672" />
...
```
##  [1] 0.8907143 0.8962698 0.9218254 0.9496032 0.9162698 0.9607143 0.9384921
##  [8] 0.9496032 0.9496032 0.9353175 0.9353175 0.9353175 0.9210317 0.8996429
## [15] 0.9339286 0.8939286 0.8796429 0.8796429
```

Vidíme, že nejlépe vychází hodnota parametru $K$ jako 6 s hodnotou přesnosti spočtenou pomocí 10-násobné CV 0.9607.
...
<div class="figure">
<img src="05-Application_1_files/figure-html/unnamed-chunk-15-1.png" alt="Závislost validační chybovosti na hodnotě $K$, tedy na počtu sousedů." width="672" />
<p class="caption">(\#fig:unnamed-chunk-15)Závislost validační chybovosti na hodnotě $K$, tedy na počtu sousedů.</p>
</div>
...
## with highest probability of correct classification max.prob=  0.9384615 
## 
## -Probability of correct classification:  0.9385
```
...
Vidíme tedy, že přesnost modelu sestrojeného pomocí metody $K$ nejbližších sousedů s optimální volbou $K_{optimal}$ rovnou 6, kterou jsme určili cross-validací, je na trénovacích datech rovna 0.0615 a na testovacích datech 0.
...
V tomto konkrétním případě jsme za počet hlavních komponent vzali $p$ = 2, které dohromady vysvětlují 94.72 % variability v datech.
První hlavní komponenta potom vysvětluje 78.75 % a druhá 15.98 % variability.
...
<div class="figure">
<img src="05-Application_1_files/figure-html/unnamed-chunk-19-1.png" alt="Hodnoty skórů prvních dvou hlavních komponent pro trénovací data. Barevně jsou odlišeny body podle příslušnosti do klasifikační třídy." width="672" />
<p class="caption">(\#fig:unnamed-chunk-19)Hodnoty skórů prvních dvou hlavních komponent pro trénovací data. Barevně jsou odlišeny body podle příslušnosti do klasifikační třídy.</p>
</div>
...
Spočítali jsme jednak přesnost klasifikátoru na trénovacích (96.92 %), tak i na testovacích datech (100 %).
...
<div class="figure">
<img src="05-Application_1_files/figure-html/unnamed-chunk-22-1.png" alt="Skóre prvních dvou hlavních komponent, barevně odlišené podle příslušnosti do klasifikační třídy. Černě je vyznačena dělící hranice (přímka v rovině prvních dvou hlavních komponent) mezi třídami sestrojená pomocí LDA." width="672" />
<p class="caption">(\#fig:unnamed-chunk-22)Skóre prvních dvou hlavních komponent, barevně odlišené podle příslušnosti do klasifikační třídy. Černě je vyznačena dělící hranice (přímka v rovině prvních dvou hlavních komponent) mezi třídami sestrojená pomocí LDA.</p>
</div>
...
Spočítali jsme tedy jednak přesnost klasifikátoru na trénovacích (95.38 %), tak i na testovacích datech (100 %).
...
<div class="figure">
<img src="05-Application_1_files/figure-html/unnamed-chunk-28-1.png" alt="Skóre prvních dvou hlavních komponent, barevně odlišené podle příslušnosti do klasifikační třídy. Černě je vyznačena dělící hranice (parabola v rovině prvních dvou hlavních komponent) mezi třídami sestrojená pomocí QDA." width="672" />
<p class="caption">(\#fig:unnamed-chunk-28)Skóre prvních dvou hlavních komponent, barevně odlišené podle příslušnosti do klasifikační třídy. Černě je vyznačena dělící hranice (parabola v rovině prvních dvou hlavních komponent) mezi třídami sestrojená pomocí QDA.</p>
</div>
...
Naopak optimální volbou na základě testovací chybovosti je $n$ rovno `NA`, tedy výrazně menší hodnota než 50.
...
Vykresleme si ještě průběh validační chybovosti i se zvýrazněnou optimální hodnotou $n_{optimal}$ rovnou `NA` s validační chybovostí `NA`.
...
Nyní již tedy můžeme definovat finální model pomocí funkcionální logistické regrese, přičemž bázi pro $\beta(t)$ volíme B-splinovou bázi s `NA` bázemi.
...
Spočítali jsme trénovací chybovost (rovna `NA` %) i testovací chybovost (rovna `NA` %).
...
Spočítali jsme tedy přesnost klasifikátoru na trénovacích (95.38 %) i na testovacích datech (100 %).
...
t.seq <- seq(rangeval[1], rangeval[2], length = 51)
...
Přesnost klasifikátoru na testovacích datech je tedy 85.71 % a na trénovacích datech 89.23 %.
...
<div class="figure">
<img src="05-Application_1_files/figure-html/unnamed-chunk-49-1.png" alt="Grafické znázornění neprořezaného rozhodovacího stromu. Modrými odstíny jsou vykresleny uzly příslušející klasifikační třídě 1 a červenými odstíny třídě 0." width="672" />
...
<div class="figure">
<img src="05-Application_1_files/figure-html/unnamed-chunk-50-1.png" alt="Finální prořezaný rozhodovací strom." width="672" />
...
Přesnost rozhodovacího stromu na testovacích datech je tedy 89.29 % a na trénovacích datech 93.85 %.
...
<div class="figure">
<img src="05-Application_1_files/figure-html/unnamed-chunk-53-1.png" alt="Grafické znázornění neprořezaného rozhodovacího stromu sestrojeného na skórech hlavních komponent. Modrými odstíny jsou vykresleny uzly příslušející klasifikační třídě 1 a červenými odstíny třídě 0." width="672" />
...
<div class="figure">
<img src="05-Application_1_files/figure-html/unnamed-chunk-54-1.png" alt="Finální prořezaný rozhodovací strom." width="672" />
...
Přesnost rozhodovacího stromu na trénovacích datech je tedy 89.23 % a na testovacích datech 85.71 %.
...
<div class="figure">
<img src="05-Application_1_files/figure-html/unnamed-chunk-58-1.png" alt="Grafické znázornění neprořezaného rozhodovacího stromu sestrojeného na bázových koeficientech. Modrými odstíny jsou vykresleny uzly příslušející klasifikační třídě 1 a červenými odstíny třídě 0." width="672" />
...
<div class="figure">
<img src="05-Application_1_files/figure-html/unnamed-chunk-59-1.png" alt="Finální prořezaný rozhodovací strom." width="672" />
...
Přesnost rozhodovacího stromu na trénovacích datech je tedy 98.46 % a na testovacích datech 92.86 %.
...
Přesnost tohoto klasifikátoru na trénovacích datech je 98.46 % a na testovacích datech 89.29 %.
...
Přesnost metody SVM na trénovacích datech je tedy 95.38 % pro lineární jádro, 83.08 % pro polynomiální jádro a 96.92 % pro gaussovské jádro.
Na testovacích datech je potom přesnost metody 96.43 % pro lineární jádro, 78.57 % pro polynomiální jádro a 100 % pro radiální jádro.
...
Přesnost metody SVM aplikované na skóre hlavních komponent na trénovacích datech je tedy 96.92 % pro lineární jádro, 95.38 % pro polynomiální jádro a 96.92 % pro gaussovské jádro.
Na testovacích datech je potom přesnost metody 100 % pro lineární jádro, 85.71 % pro polynomiální jádro a 100 % pro radiální jádro.
...
<div class="figure">
<img src="05-Application_1_files/figure-html/unnamed-chunk-74-1.png" alt="Skóre prvních dvou hlavních komponent, barevně odlišené podle příslušnosti do klasifikační třídy. Černě je vyznačena dělící hranice (přímka, resp. křivky v rovině prvních dvou hlavních komponent) mezi třídami sestrojená pomocí metody SVM." width="672" />
<p class="caption">(\#fig:unnamed-chunk-74)Skóre prvních dvou hlavních komponent, barevně odlišené podle příslušnosti do klasifikační třídy. Černě je vyznačena dělící hranice (přímka, resp. křivky v rovině prvních dvou hlavních komponent) mezi třídami sestrojená pomocí metody SVM.</p>
</div>
...
Přesnost metody SVM aplikované na bázové koeficienty na trénovacích datech je tedy 96.92 % pro lineární jádro, 84.62 % pro polynomiální jádro a 96.92 % pro gaussovské jádro.
Na testovacích datech je potom přesnost metody 100 % pro lineární jádro, 82.14 % pro polynomiální jádro a 100 % pro radiální jádro.
...
Vidíme, že nejlépe vychází hodnota parametru $d$ jako 8 pro lineární jádro s hodnotou přesnosti spočtenou pomocí 10-násobné CV 0.9514, 7 pro polynomiální jádro s hodnotou přesnosti spočtenou pomocí 10-násobné CV 0.9165 a 6 pro radiální jádro s hodnotou přesnosti 0.9314.
...
<div class="figure">
<img src="05-Application_1_files/figure-html/unnamed-chunk-79-1.png" alt="Závislost validační chybovosti na dimenzi podprostoru $V_d$, zvlášť pro všechna tři uvažovaná jádra v metodě SVM. Černými body jsou vyznačeny optimální hodnoty dimenze $V_d$ pro jednotlivé jádrové funkce." width="672" />
<p class="caption">(\#fig:unnamed-chunk-79)Závislost validační chybovosti na dimenzi podprostoru $V_d$, zvlášť pro všechna tři uvažovaná jádra v metodě SVM. Černými body jsou vyznačeny optimální hodnoty dimenze $V_d$ pro jednotlivé jádrové funkce.</p>
</div>
...
Přesnost metody SVM aplikované na bázové koeficienty na trénovacích datech je tedy 3.08 % pro lineární jádro, 3.08 % pro polynomiální jádro a 4.62 % pro gaussovské jádro.
Na testovacích datech je potom přesnost metody 3.57 % pro lineární jádro, 0 % pro polynomiální jádro a 0 % pro radiální jádro.
...
Nejprve zvolme za $\hat d$ celou dimenzi, tedy $\hat d = m ={}$ 51, následně určíme optimální $\hat d$ pomocí cross-validace.
...
Table: (\#tab:unnamed-chunk-87)Souhrnné výsledky metody SVM v kombinaci s RKHS na simulovaných datech. $\widehat{Err}_{train}$ značí odhad trénovací chybovosti a $\widehat{Err}_{test}$ testovací chybovosti.

Model                                $\widehat{Err}_{train}\quad\quad\quad\quad\quad$         $\widehat{Err}_{test}\quad\quad\quad\quad\quad$       
----------------------------------  -------------------------------------------------------  -------------------------------------------------------
SVM linear - RKHS                                                                    0.0000                                                   0.2143
SVM poly - RKHS                                                                      0.0769                                                   0.3214
SVM rbf - RKHS                                                                       0.0154                                                   0.0714
...
# spocitame prumerne presnosti pro jednotliva d pres folds
for (n_method in 1:length(CV.results)) {
  CV.results[[n_method]] <- apply(CV.results[[n_method]], c(1, 2), mean, na.rm = TRUE)
}
...
err.opt.cv <- c(min(CV.results$SVM.l, na.rm = TRUE), 
                     min(CV.results$SVM.p, na.rm = TRUE),
                     min(CV.results$SVM.r, na.rm = TRUE))
...
Table: (\#tab:unnamed-chunk-90)Souhrnné výsledky cross-validace pro metodu SVM v kombinaci s RKHS na simulovaných datech. $\widehat{Err}_{train}$ značí odhad trénovací chybovosti a $\widehat{Err}_{test}$ testovací chybovosti.

          $\quad\quad\quad\quad\quad d$   $\quad\quad\quad\quad\quad\gamma$   $\widehat{Err}_{cross\_validace}$  Model                             
-------  ------------------------------  ----------------------------------  ----------------------------------  ----------------------------------
linear                               12                              0.1389                              0.0310  linear                            
poly                                  3                              0.2683                              0.0595  polynomial                        
radial                                4                              0.1000                              0.0411  radial                            

Vidíme, že nejlépe vychází hodnota parametru $d={}$ 12 a $\gamma={}$ 0.1389 pro lineární jádro s hodnotou přesnosti spočtenou pomocí 10-násobné CV 0.969, $d={}$ 3 a $\gamma={}$ 0.2683 pro polynomiální jádro s hodnotou přesnosti spočtenou pomocí 10-násobné CV 0.9405 a $d={}$ 4 a $\gamma={}$ 0.1 pro radiální jádro s hodnotou přesnosti 0.9589.
...
<div class="figure">
<img src="05-Application_1_files/figure-html/unnamed-chunk-91-1.png" alt="Závislost validační chybovosti na volbě hyperparametrů $d$ a $\gamma$, zvlášť pro všechna tři uvažovaná jádra v metodě SVM." width="672" />
<p class="caption">(\#fig:unnamed-chunk-91)Závislost validační chybovosti na volbě hyperparametrů $d$ a $\gamma$, zvlášť pro všechna tři uvažovaná jádra v metodě SVM.</p>
</div>
...
Table: (\#tab:unnamed-chunk-94)Souhrnné výsledky metody SVM v kombinaci s RKHS na simulovaných datech. $\widehat{Err}_{train}$ značí odhad trénovací chybovosti a $\widehat{Err}_{test}$ testovací chybovosti.

Model                                $\widehat{Err}_{train}\quad\quad\quad\quad\quad$         $\widehat{Err}_{test}\quad\quad\quad\quad\quad$       
----------------------------------  -------------------------------------------------------  -------------------------------------------------------
SVM linear - RKHS - radial                                                           0.0308                                                   0.0357
SVM poly - RKHS - radial                                                             0.0615                                                   0.2143
SVM rbf - RKHS - radial                                                              0.0308                                                   0.0357

Přesnost metody SVM v kombinaci s projekcí na Reproducing Kernel Hilbert Space je tedy na trénovacích datech rovna 3.08 % pro lineární jádro, 6.15 % pro polynomiální jádro a 3.08 % pro gaussovské jádro.
Na testovacích datech je potom přesnost metody 3.57 % pro lineární jádro, 21.43 % pro polynomiální jádro a 3.57 % pro radiální jádro.
...
# spocitame prumerne presnosti pro jednotliva d pres folds
for (n_method in 1:length(CV.results)) {
  CV.results[[n_method]] <- apply(CV.results[[n_method]], 1, mean, na.rm = TRUE)
}
...
Table: (\#tab:unnamed-chunk-109)Souhrnné výsledky cross-validace pro metodu SVM v kombinaci s RKHS na simulovaných datech. $\widehat{Err}_{train}$ značí odhad trénovací chybovosti a $\widehat{Err}_{test}$ testovací chybovosti.

          $\quad\quad\quad\quad\quad d$   $\widehat{Err}_{cross\_validace}$  Model                             
-------  ------------------------------  ----------------------------------  ----------------------------------
linear                               19                              0.1006  linear                            
poly                                 17                              0.1315  polynomial                        
radial                               19                              0.0720  radial                            

Vidíme, že nejlépe vychází hodnota parametru $d={}$ 19 pro lineární jádro s hodnotou přesnosti spočtenou pomocí 10-násobné CV 0.8994, $d={}$ 17 pro polynomiální jádro s hodnotou přesnosti spočtenou pomocí 10-násobné CV 0.8685 a $d={}$ 19 pro radiální jádro s hodnotou přesnosti 0.928.
...
Table: (\#tab:unnamed-chunk-112)Souhrnné výsledky metody SVM v kombinaci s RKHS na simulovaných datech. $\widehat{Err}_{train}$ značí odhad trénovací chybovosti a $\widehat{Err}_{test}$ testovací chybovosti.

Model                                $\widehat{Err}_{train}\quad\quad\quad\quad\quad$         $\widehat{Err}_{test}\quad\quad\quad\quad\quad$       
----------------------------------  -------------------------------------------------------  -------------------------------------------------------
SVM linear - RKHS - linear                                                           0.0462                                                   0.1071
SVM poly - RKHS - linear                                                             0.0615                                                   0.0714
SVM rbf - RKHS - linear                                                              0.0462                                                   0.0714

Přesnost metody SVM v kombinaci s projekcí na Reproducing Kernel Hilbert Space je tedy na trénovacích datech rovna 4.62 % pro lineární jádro, 6.15 % pro polynomiální jádro a 4.62 % pro gaussovské jádro.
Na testovacích datech je potom přesnost metody 10.71 % pro lineární jádro, 7.14 % pro polynomiální jádro a 7.14 % pro radiální jádro.
...
Table: (\#tab:unnamed-chunk-114)Souhrnné výsledky použitých metod na simulovaných datech. $\widehat{Err}_{train}$ značí odhad trénovací chybovosti a $\widehat{Err}_{test}$ testovací chybovosti.
...
LDA                                                                                  0.0308                                                   0.0000
QDA                                                                                  0.0462                                                   0.0000
LR score                                                                             0.0462                                                   0.0000
Tree - diskr.                                                                        0.1077                                                   0.1429
Tree - score                                                                         0.0615                                                   0.1071
Tree - Bbasis                                                                        0.1077                                                   0.1429
RForest - diskr                                                                      0.0154                                                   0.0357
RForest - score                                                                      0.0154                                                   0.0714
RForest - Bbasis                                                                     0.0154                                                   0.1071
SVM linear - diskr                                                                   0.0462                                                   0.0357
SVM poly - diskr                                                                     0.1692                                                   0.2143
SVM rbf - diskr                                                                      0.0308                                                   0.0000
SVM linear - PCA                                                                     0.0308                                                   0.0000
SVM poly - PCA                                                                       0.0462                                                   0.1429
SVM rbf - PCA                                                                        0.0308                                                   0.0000
SVM linear - Bbasis                                                                  0.0308                                                   0.0000
SVM poly - Bbasis                                                                    0.1538                                                   0.1786
SVM rbf - Bbasis                                                                     0.0308                                                   0.0000
SVM linear - projection                                                              0.0308                                                   0.0357
SVM poly - projection                                                                0.0308                                                   0.0000
SVM rbf - projection                                                                 0.0462                                                   0.0000
SVM linear - RKHS - radial                                                           0.0308                                                   0.0357
SVM poly - RKHS - radial                                                             0.0615                                                   0.2143
SVM rbf - RKHS - radial                                                              0.0308                                                   0.0357
SVM linear - RKHS - linear                                                           0.0462                                                   0.1071
...
methods <- c('KNN', 'LDA', 'QDA', 'LR_score', 'Tree_discr',
             ...
             'SVM rbf - projection', 'SVM linear - RKHS - radial', 'SVM poly - RKHS - radial', 'SVM rbf - RKHS - radial', 'SVM linear - RKHS - linear', 'SVM poly - RKHS - linear', 'SVM rbf - RKHS - linear')
...
for(sim in 1:n.sim) {
  ...
  # hodnoty hyperparametru, ktere budeme prochazet
  dimensions <- seq(3, 20, by =2) # rozumny rozsah hodnot d
...
Table: (\#tab:unnamed-chunk-118)Souhrnné výsledky použitých metod na simulovaných datech. $\widehat{Err}_{train}$ značí odhad trénovací chybovosti, $\widehat{Err}_{test}$ testovací chybovosti, $\widehat{SD}_{train}$ odhad směrodatné odchylky trénovacích chybovostí a $\widehat{SD}_{test}$ je odhad směrodatné odchylky testovacích chybovostí.

                              $\widehat{Err}_{train}$   $\widehat{Err}_{test}$   $\widehat{SD}_{train}$   $\widehat{SD}_{test}$
---------------------------  ------------------------  -----------------------  -----------------------  ----------------------
KNN                                            0.0615                   0.0357                       NA                      NA
LDA                                            0.0154                   0.0357                       NA                      NA
QDA                                            0.0154                   0.0357                       NA                      NA
LR_score                                       0.0154                   0.0357                       NA                      NA
Tree_discr                                     0.1385                   0.0714                       NA                      NA
Tree_score                                     0.0923                   0.1071                       NA                      NA
Tree_Bbasis                                    0.1385                   0.0714                       NA                      NA
RF_discr                                       0.0154                   0.0357                       NA                      NA
RF_score                                       0.0154                   0.0357                       NA                      NA
RF_Bbasis                                      0.0154                   0.0357                       NA                      NA
SVM linear - diskr                             0.0154                   0.0357                       NA                      NA
SVM poly - diskr                               0.1231                   0.0714                       NA                      NA
SVM rbf - diskr                                0.0154                   0.0357                       NA                      NA
SVM linear - PCA                               0.0154                   0.0357                       NA                      NA
SVM poly - PCA                                 0.0462                   0.0000                       NA                      NA
SVM rbf - PCA                                  0.0154                   0.0357                       NA                      NA
SVM linear - Bbasis                            0.0154                   0.0357                       NA                      NA
SVM poly - Bbasis                              0.1385                   0.1786                       NA                      NA
SVM rbf - Bbasis                               0.0154                   0.0357                       NA                      NA
SVM linear - projection                        0.0154                   0.0357                       NA                      NA
SVM poly - projection                          0.0154                   0.0357                       NA                      NA
SVM rbf - projection                           0.0154                   0.0357                       NA                      NA
SVM linear - RKHS - radial                     0.0615                   0.1071                       NA                      NA
SVM poly - RKHS - radial                       0.0462                   0.0357                       NA                      NA
SVM rbf - RKHS - radial                        0.0462                   0.1071                       NA                      NA
SVM linear - RKHS - linear                     0.0923                   0.1071                       NA                      NA
SVM poly - RKHS - linear                       0.0923                   0.1429                       NA                      NA
SVM rbf - RKHS - linear                        0.0615                   0.0357                       NA                      NA
...
<div class="figure">
<img src="05-Application_1_files/figure-html/unnamed-chunk-119-1.png" alt="Krabicové diagramy trénovacích chybovostí pro 100 simulací zvlášť pro jednotlivé klasifikační metody. Černými symboly $+$ jsou vyznačeny průměry." width="672" />
<p class="caption">(\#fig:unnamed-chunk-119)Krabicové diagramy trénovacích chybovostí pro 100 simulací zvlášť pro jednotlivé klasifikační metody. Černými symboly $+$ jsou vyznačeny průměry.</p>
</div>
...
<div class="figure">
<img src="05-Application_1_files/figure-html/unnamed-chunk-121-1.png" alt="Krabicové diagramy testovacích chybovostí pro 100 simulací zvlášť pro jednotlivé klasifikační metody. Černými symboly $+$ jsou vyznačeny průměry." width="672" />
<p class="caption">(\#fig:unnamed-chunk-121)Krabicové diagramy testovacích chybovostí pro 100 simulací zvlášť pro jednotlivé klasifikační metody. Černými symboly $+$ jsou vyznačeny průměry.</p>
</div>
...
Table: (\#tab:unnamed-chunk-123)Mediány hodnot hyperparametrů pro vybrané metody, u nichž se určoval nějaký hyperparametr pomocí cross-validace.
...
<div class="figure">
<img src="05-Application_1_files/figure-html/unnamed-chunk-124-1.png" alt="Krabicové diagramy hodnot hyperparametrů." width="672" />
<p class="caption">(\#fig:unnamed-chunk-124)Krabicové diagramy hodnot hyperparametrů.</p>
</div>
