...
author: "Tomáš Pompa"
date: "`r paste0(Sys.Date() |> substr(9, 10), '. ', Sys.Date() |> substr(6, 7), '. ', Sys.Date() |> substr(1,4))`"
...
```{r echo=FALSE, eval=T}
ggsave('boxplot_train.pdf', width = 10, height = 6)
```
...
```{r fig.cap='Krabicové diagramy hodnot hyperparametrů.', eval=T, warning=FALSE, message=FALSE}
CV_RESULTS |> 
...
```{r}
# vykresleni polynomu
xx <- seq(min(xx.0), max(xx.1), length = 501)
yy.0 <- poly.0(xx)
yy.1 <- poly.1(xx)

dat_poly_plot <- data.frame()
```
...
```{r eval=FALSE, fig.cap='Závislost testovací a trénovací chybovosti na počtu bázových funkcí pro $\\beta$. Červeným bodem je znázorněn optimální počet $n_{optimal}$ zvolený jako minimum testovací chybovosti, černou čarou je vykreslena testovací a modrou přerušovanou čarou je vykreslen průběh trénovací chybovosti.'}
n.basis.beta.opt <- pred.baz$n.basis[which.min(pred.baz$Err.test)]
...
```{r fig.cap='Závislost validační chybovosti na hodnotě $n_{basis}$, tedy na počtu bází.', eval=FALSE}
CV.results <- data.frame(n.basis = n.basis, CV = CV.results)
CV.results |> ggplot(aes(x = n.basis, y = 1 - CV)) + 
  geom_line(linetype = 'dashed', colour = 'grey') + 
  geom_point(size = 1.5) + 
...
```{r fig.cap='Závoslost odhadnutých pravděpodobností na hodnotách lineárního prediktoru. Barevně jsou odlišeny body podle příslušnosti do klasifikační třídy.', eval=FALSE}
data.frame(
  linear.predictor = model.glm$linear.predictors,
  response = model.glm$fitted.values,
  Y = factor(y.train)
...
```{r fig.cap='Průběh odhadu parametrické funkce $\\beta(t), t \\in [0, 1]$.', eval=FALSE}
t.seq <- seq(0, 1, length = 1001)
beta.seq <- eval.fd(evalarg = t.seq, fdobj = model.glm$beta.l$x)
...
```{r CV_SVM_RKHS_r, warning=FALSE, message=FALSE}
# samotna CV
for (gamma in gamma.cv) {
  K <- Kernel.RKHS(t.seq, gamma = gamma)
  Eig <- eigen(K)
...
```{r CV_SVM_RKHS_p, warning=FALSE, message=FALSE, eval=FALSE}
# samotna CV
for (p in poly.cv) {
  K <- Kernel.RKHS(t.seq, p = p)
  Eig <- eigen(K)
...
<!--chapter:end:05-Application_1.Rmd-->
...
p3 <- ggplot(data = data.frame(x = c(handwritTime, handwritTime),
  ...
  theme(aspect.ratio = 1,
        legend.position = 'none')
(p1 | p2 | p3)
# ggsave('C:/Users/Tom/OneDrive - MUNI/Diplomka/R/fda_handwrit_samples.pdf',
#        width = 12, height = 4)
```
...
data.frame(x = rep(data.gr$t, 2),
           y = c(data.gr$H1, data.gr$H2),
...
<!--chapter:end:06-Application_2.Rmd-->
...
<!--chapter:end:07-Application_3.Rmd-->
...
<!--chapter:end:08-Application_4.Rmd-->
