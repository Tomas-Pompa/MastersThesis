...
```{r fig.cap="Růstové křivky zlášť pro dívky a chlapce."}
...
```{r fig.cap='Vykreslení všech vyhlazených pozorovaných křivek, barevně jsou odlišeny křivky podle pohlaví. Černou čerchovanou čarou je zakreslen průměr pro každou třídu.', eval=T}
n <- n_girls + n_boys
...
Vidíme, že v obou množinách (testovací a trénovací) jsou chlapci i dívky zastoupeni přibližně ve stejném poměru, přičemž bereme do úvahy, že chlapců je méně než dívek. Konkrétně dívek je přibližně `r round(c(n_girls) / n, 4) * 100` $\%$ a chlapců `r round(c(n_boys) / n, 4) * 100` $\%$.
...
```{r fig.cap="Skóre prvních dvou hlavních komponent, barevně odlišené podle pohlaví. Černě je vyznačena dělící hranice (přímka v rovině prvních dvou hlavních komponent) mezi třídami sestrojená pomocí LDA."}
...
```{r fig.cap="Skóre prvních dvou hlavních komponent, barevně odlišené podle pohlaví. Černě je vyznačena dělící hranice (parabola v rovině prvních dvou hlavních komponent) mezi třídami sestrojená pomocí QDA."}
...
```{r, echo=FALSE}
load('RData/aplikace_01.RData', verbose = F)
load('RData/aplikace_01_res.RData', verbose = F)
```
...
```{r echo=FALSE, eval=T}
ggsave('figures/app_01_boxplot_train.pdf', width = 10, height = 6)
```
...
```{r echo=FALSE, eval=T}
ggsave('figures/app_01_boxplot_test.pdf', width = 10, height = 6)
```
...
```{r echo=FALSE, eval=T}
ggsave('figures/app_01_histogram_hyperparams_0.pdf', width = 10, height = 6)
```
...
```{r echo=FALSE, eval=T}
ggsave('figures/app_01_histogram_hyperparams_1.pdf', width = 10, height = 6)
```
...
```{r echo=FALSE, eval=T}
ggsave('figures/app_01_histogram_hyperparams_2.pdf', width = 10, height = 6)
```
...
```{r echo=FALSE, eval=T}
ggsave('figures/app_01_histogram_hyperparams_3.pdf', width = 10, height = 6)
```
...
```{r echo=FALSE, eval=T}
ggsave('figures/app_01_histogram_hyperparams_4.pdf', width = 10, height = 6)
```
