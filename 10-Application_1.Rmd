...
```{r fig.cap='Průběh odhadu parametrické funkce $\\beta(t), t \\in [1, 18]$.'}
...
Chybovost metody SVM aplikované na bázové koeficienty na trénovacích datech je tedy `r 100 * round(1 - Res[1, 2], 4)` % pro lineární jádro, `r 100 * round(1 - Res[2, 2], 4)` % pro polynomiální jádro a `r 100 * round(1 - Res[3, 2], 4)` % pro gaussovské jádro.
Na testovacích datech je potom chybovost metody `r 100 * round(1 - Res[1, 3], 4)` % pro lineární jádro, `r 100 * round(1 - Res[2, 3], 4)` % pro polynomiální jádro a `r 100 * round(1 - Res[3, 3], 4)` % pro radiální jádro.
...
```{r fig.cap='Závislost validační chybovosti na volbě hyperparametrů $d$ a $\\gamma$, zvlášť pro všechna tři uvažovaná jádra v metodě SVM.', warning=FALSE}
...
Vidíme, že nejlépe vychází hodnota parametru $d={}$ `r d.opt[1]` pro lineární jádro s hodnotou chybovosti spočtenou pomocí 10-násobné CV `r round(err.opt.cv[1], 4)`, $d={}$ `r d.opt[2]` pro polynomiální jádro s hodnotou spočtenou pomocí 10-násobné CV `r round(err.opt.cv[2], 4)` a $d={}$ `r d.opt[3]` pro radiální jádro s hodnotou `r round(err.opt.cv[3], 4)`.
